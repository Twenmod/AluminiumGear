...
//Custom function to override projection
void ImGui_ImplOpenGL3_SetupMVP(const glm::mat4& MVP)
{
	ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
	bd->MVP = MVP;
}
unsigned int ImGui_ImplOpenGL3_GetShader()
{
	ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
	return bd->ShaderHandle;
}
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ...
    //glUniformMatrix4fv(bd->AttribLocationProjMtx, 1, GL_FALSE, &ortho_projection[0][0]); // We do this manually using my function
...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
		...
		if (draw_list->draw3d)
		{
			glUseProgram(bd->ShaderHandle);
			glUniform1i(bd->AttribLocationTex, 0);
			glUniformMatrix4fv(bd->AttribLocationProjMtx, 1, GL_FALSE, &bd->MVP[0][0]);
			glDisable(GL_SCISSOR_TEST); //Dont do scissor test since we dont know where window will end up
		}
		else
		{
...
bool    ImGui_ImplOpenGL3_CreateDeviceObjects()
{
	...
	const GLchar* vertex_shader_glsl_120 = // WE'RE USING THIS SHADER
        ...
        "    Frag_Color = Color;\n"
        "    Frag_POS = Position;\n"
        "    gl_Position = ProjMtx * vec4(Position.x + mod(Position.y+time,2.0)*0.4,Position.y,0,1);\n"
        "}\n";
    ...
    const GLchar* fragment_shader_glsl_120 = // WE'RE USING THIS SHADER
        "#ifdef GL_ES\n"
		...
		"float animationSpeed = -3.5;\n"
		"float lineGrid(vec2 pos) {\n"
		"    float v = step(0.8,mod((pos.y+Time*animationSpeed), 4.0));\n"
		"    return v+baseOpacity;\n"
		"}\n"
        "void main()\n"
        "{\n"
        "    vec4 color = Frag_Color * texture2D(Texture, Frag_UV.st);\n"
		"    color = vec4(vec3(dot(color.rgb,vec3(0.3,0.59,0.11))),color.a);\n"
		"    color *= vec4(0.4,0.6,0.9,1.0);\n"
		"    color.a *= lineGrid(Frag_POS+Frag_UV);\n"
		"    gl_FragColor = color;\n"
        "}\n";
